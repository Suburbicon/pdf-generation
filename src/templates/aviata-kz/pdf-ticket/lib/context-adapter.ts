...
function getDurationTranslationContext(h: number, m: number) {
  if (h === 0 && m !== 0) return 'minutes';
  if (h !== 0 && m === 0) return 'hours';
  return '';
}
...
class FlightMapper {
  ...
  private getDuration(d1: string, d2: string) {
    ...
    const context = getDurationTranslationContext(h, m);
...
class Pricing {
  ...
  private getTotal() {
    return sumBy(this.data.pricing, item => Number(item.amount));
  }
    ...
    return Object.entries(productGroups).flatMap(([key, group]) => {
      if (separateProductTypes.has(key)) {
        return group.map(product =>
          createGrouppedProduct(product.title, Number(product.amount), 1)
        );
      }

      const totalAmount = sumBy(group, item => Number(item.amount));
      return createGrouppedProduct(group[0].title, totalAmount, group.length);
    });
  }
  ...
  private groupTickets(tickets: PdfTicketRequestSchemaType['pricing']) {
    ...
    return map(productGroups, group => {
      ...
      return {
        title: pax.full_name,
        price: renderPrice(Number(item.amount)),
        count: group.length,
        amount: renderPrice(sumBy(group, item => Number(item.amount))),
      };
    });
  }
...
function sortFlights(
  ...
  return flights.sort((first, second) => {
    // @ts-expect-error Date object is also valid
    return first.firstSegmentDepAt - second.firstSegmentDepAt;
  });
}

function createGrouppedProduct(title: string, amount: number, count: number) {
  return {
    amount: renderPrice(amount),
    price: renderPrice(amount / count),
    title,
    count,
  };
}
