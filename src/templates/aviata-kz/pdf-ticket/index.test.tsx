...
describe('aviata/pdf-ticket', () => {
  ...
  it('Base OW with 24 hour flight duration', () => {
    return expectPdfToMatchImageSnapshot(
      import(
        '~/src/__mocks__/pdf-ticket-payload/base_OW_one_day_duration_difference.json'
      )
    );
  });
  ...
  it('separate ticket pricing for passengers with similar full names', () => {
    return expectPdfToMatchImageSnapshot(
      import(
        '~/src/__mocks__/pdf-ticket-payload/pricing_similar_pax_names.json'
      )
    );
  });

  it('nullable ticket number', () => {
    return expectPdfToMatchImageSnapshot(
      import('~/src/__mocks__/pdf-ticket-payload/nullable_ticket_number.json')
    );
  });

  describe('Exchanges', () => {
    it('Exchanged OW with 2 ADT', () => {
      return expectPdfToMatchImageSnapshot(
        import('~/src/__mocks__/pdf-ticket-payload/exchanged_OW_2_ADT.json')
      );
    });

    it('Exchanged RT with 2 ADT', () => {
      return expectPdfToMatchImageSnapshot(
        import('~/src/__mocks__/pdf-ticket-payload/exchanged_RT_2_ADT.json')
      );
    });

    it('Exchanged OW with the new booking and 3 ADT', () => {
      return expectPdfToMatchImageSnapshot(
        import(
          '~/src/__mocks__/pdf-ticket-payload/exchanged_OW_new_booking_3_ADT.json'
        )
      );
    });

    it('Exchanged RT with the new booking and 3 ADT', () => {
      return expectPdfToMatchImageSnapshot(
        import(
          '~/src/__mocks__/pdf-ticket-payload/exchanged_RT_new_booking_3_ADT.json'
        )
      );
    });
  });
