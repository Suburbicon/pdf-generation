...
import { PdfBusinessTripWritRequestSchema } from '~/src/shared/lib/schema/index.ts';
...
export const PdfBusinessTripWrit: FastifyPluginCallback = (
  fastify,
  _,
  done
) => {
  ...
  fastify.withTypeProvider<ZodTypeProvider>().route({
    url: '/business-trip/order/render',
    method: 'POST',
    schema: {
      tags: ['PDF'],
      description: 'Render customer PDF route receipt',
      produces: ['application/pdf', 'application/json'],
      body: PdfBusinessTripWritRequestSchema,
    ...
    async handler(req, reply) {
      const eventId = fastify.Sentry.captureMessage(
        'Incoming request to render pdf business trip writ',
        {
          tags: {
      ...
      try {
        ...
        const error = new Error('ERR_RENDER_PDF_BUSINESS_TRIP_WRIT', { cause });
    ...
    onResponse(req, reply, done) {
      const endpoint = `pdf_business_trip_writ_${snakeCase(
