...
export const emailBookRoute: FastifyPluginCallback = (fastify, _, done) => {
  ...
  fastify.withTypeProvider<ZodTypeProvider>().route({
    ...
    schema: {
      tags: ['EMAIL'],
      description: 'Render customer Email receipt for created booking',
      produces: ['text/html'],
      headers: HeadersSchema,
      body: BookOrderSchema,
      response: {
        200: z.unknown(),
      ...
      try {
        const getServiceEmail = (): React.ReactElement => {
          switch (req.headers['x-tenant']) {
            case TENANT.B2B_KZ:
              return <B2B.BookEmail order={req.body} i18n={req.i18n} />;
            default:
    ...
    onResponse(req, reply, done) {
      const endpoint = `book_email_${snakeCase(req.headers['x-tenant'])}`;
